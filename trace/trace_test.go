...
func TestStartFinishConsumerSpan(t *testing.T) {
	...
	sp, ctx := ConsumerSpan(context.Background(), AMQPConsumerComponent, hdr)
	assert.NotNil(sp)
	assert.NotNil(ctx)
	assert.IsType(&mocktracer.MockSpan{}, sp)
	jsp := sp.(*mocktracer.MockSpan)
	assert.NotNil(jsp)
	assert.Equal(AMQPConsumerComponent, jsp.OperationName)
	SpanError(sp)
	assert.NotNil(sp)
...
func TestStartFinishChildSpan(t *testing.T) {
	...
	mtr := mocktracer.New()
	opentracing.SetGlobalTracer(mtr)
	sp, ctx := ConsumerSpan(context.Background(), AMQPConsumerComponent, nil)
	assert.NotNil(sp)
	assert.NotNil(ctx)
	childSp, childCtx := ChildSpan(ctx, "cmp", opentracing.Tag{Key: "key", Value: "value"})
	...
	assert.NotNil(jsp)
	assert.Equal("cmp", jsp.OperationName)
	SpanError(childSp)
	assert.NotNil(childSp)
	rawSpan := mtr.FinishedSpans()[0]
	assert.Equal(map[string]interface{}{
