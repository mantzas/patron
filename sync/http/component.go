...
func createHandler(routes []Route, infof func(string, ...interface{})) http.Handler {
	infof("adding %d routes", len(routes))
	router := httprouter.New()
	for _, route := range routes {
		router.HandlerFunc(route.Method, route.Pattern, route.Handler)
		infof("added route %s %s", route.Method, route.Pattern)
	}
	return router
}
